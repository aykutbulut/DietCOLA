...
static double checkInfeasibility(DcModel* model, const int columnNumber,
				 int & preferredWay, double & otherWay)
{
	...
	model->getDblParam(DcModel::DcIntegerTolerance);
    if (nearest > value)
	preferredWay = 1;
    else
	preferredWay = -1;
...
AlpsTreeNode*
DcTreeNode::createNewTreeNode(AlpsNodeDesc *&desc) const
{
    // Create a new tree node
    DcNodeDesc* d = dynamic_cast<DcNodeDesc*>(desc);
    DcTreeNode* node = new DcTreeNode(d);
    desc = 0;
    return(node);
}
...
int
DcTreeNode::process(bool isRoot, bool rampUp)
{
	...
	if (myType == AlpsProcessTypeHub || myType == AlpsProcessTypeMaster){
	    cutDuringRampup=model->DcPar()->entry(DcParams::cutDuringRampup);
	}
    }
    ...
    bool feasible = m->solveWithoutCuts(cuts, maximumCutPassesAtRoot,
    ...
    if (heurFound >= 0) {
	betterSolution = true;
	DcSolution* sol = new DcSolution(numCols,
					   m->bestSolution(),
					   m->getObjValue());
	...
	if(m->feasibleSolution(numberInfeasibleIntegers)) { // MIP feasible
	    if (val < bestValue) {
		betterSolution = true;
		m->setBestSolution(DC_BRANCHSOL,
				   val, m->getColSolution());
		DcSolution* ksol = new DcSolution(numCols,
						    m->getColSolution(),
						    val);
...
DcTreeNode::branch()
{
	...
	std::cout << "DcError: branchedOn_ = "<< branchedOn_ << "; numCols = "
    ...
    child = new DcNodeDesc(m, newLbs, newUbs);
...
int
DcTreeNode::chooseBranch(DcModel* model, bool& strongFound)
{
    strongFound = false;
    int i = -1;
    ...
    double maxDist = model->getDblParam(DcModel::DcIntegerTolerance);
    double value = 0.0;
    double nearest = 0.0;
    double distance = 0.0;
...
AlpsEncoded*
DcTreeNode::encode() const
{
#if defined(DC_DEBUG_MORE)
    std::cout << "DcTreeNode::encode()--start to encode" << std::endl;
#endif
    AlpsEncoded* encoded = new AlpsEncoded(AlpsKnowledgeTypeNode);
    DcNodeDesc* desc = dynamic_cast<DcNodeDesc*>(desc_);
    DcModel* model = dynamic_cast<DcModel*>(desc->getModel());
...
AlpsKnowledge*
DcTreeNode::decode(AlpsEncoded& encoded) const
{
    int expli;
    int numCols;
    double* lb;
    ...
    DcNodeDesc* nodeDesc = new DcNodeDesc(dynamic_cast<DcModel*>
					    (desc_->getModel()), lb, ub);

    //  nodeDesc->setModel(getKnowledgeBroker()->getDataPool()->getModel());
    DcTreeNode* treeNode = new DcTreeNode(nodeDesc);
...
//	       -2  Infeasible
#if 0
int DcTreeNode::chooseBranch(DcModel *model, bool& strongFound)
{
	...
	strongFound = model->setBestSolution(DC_STRONGSOL,
					     objectiveValue,
    ...
    DcBranchDecision * decision = model->branchingMethod();
    if (!decision) {
	decision = new DcBranchDefaultDecision();
    }
	...
	for (i = 0; i < numberStrong; ++i) {
		...
		if (newObjectiveValue > model->getCutoff()) {
			...
			strongFound =
			    model->setBestSolution(DC_STRONGSOL,
						   newObjectiveValue,
						   solver->getColSolution());
			if (newObjectiveValue > model->getCutoff())
			    objectiveChange = 1.0e100;  // discard it
	...
	if (anyAction > 0) {
		...
		model->messageHandler()->message(DC_STRONG,model->messages())
		    << i << iColumn
...
#if 0		// FIXME
		std::cout << "DcStrongBranch: col index : ";
		for (i = 0; i < numberStrong; ++i) {
		    std::cout<< objects[i] << " ";
		}
		std::cout <<"\nDcStrongBranch: branchedOn_ = " << branchedOn_
			  << "; branchedOnVal_ = " << branchedOnVal_
			  << std::endl;
#endif
	    }
	    else {
		std::cout << "DcError: whichObject = " << whichObject
